(ns vehiclerentalops.phase-test
  "Unit tests of `vehiclerentalops.phase` rollout logic."
  (:require [clojure.test :refer [deftest is testing]]
            [vehiclerentalops.phase :as phase]))
...
(deftest phase-3-auto-commits-clean-ops
  (testing "phase 3 auto-commits clean, high-conf non-safety-concern ops"
...
(deftest vehicle-safety-concern-holds-when-not-enabled
  (testing ":flag-vehicle-safety-concern holds in phases 0-2 (not yet enabled)"
    (doseq [ph [0 1 2]]
      (let [{:keys [disposition]} (phase/gate ph {:op :flag-vehicle-safety-concern} :escalate)]
        (is (= :hold disposition)
            (str "phase " ph " has not enabled flag-vehicle-safety-concern yet"))))))

(deftest vehicle-safety-concern-escalates-when-enabled
  (testing ":flag-vehicle-safety-concern ALWAYS escalates when enabled, even if governor says commit"
    (let [{:keys [disposition]} (phase/gate 3 {:op :flag-vehicle-safety-concern} :commit)]
      (is (= :escalate disposition)
          "phase 3 must escalate vehicle safety concerns regardless of governor disposition"))))
