(ns vehiclerentalops.store-contract-test
  "Contract tests for `vehiclerentalops.store/Store` protocol."
...
(deftest mem-store-rental-log
  (testing "MemStore commit-record! appends to rental-log"
    (let [s (store/mem-store {})
          record {:op :log-rental-record :account-id "a1" :value {:vehicle-id "CAR-1"}}]
