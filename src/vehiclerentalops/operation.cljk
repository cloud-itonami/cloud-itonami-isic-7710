(ns vehiclerentalops.operation
  "OperationActor -- one coordination request = one supervised actor
  run, expressed as a langgraph-clj StateGraph. The advisor
  (VehicleRentalAdvisor) is sealed into a single node (:advise); its
  proposal is ALWAYS routed through the VehicleRentalGovernor
  ...
  (:require [langgraph.graph :as g]
            ...
            [vehiclerentalops.governor :as governor]
...
(defn build
  ...
  `vehiclerentalops.store/Store`).
  opts:
    :advisor      -- a `vehiclerentalops.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; VehicleRentalAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; VehicleRentalGovernor -- independent censor (separate system than the advisor).
      (g/add-node :govern
